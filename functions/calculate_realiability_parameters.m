...
for p = 1:Ntrials
    ...
    Meantext{p} = sprintf ('%.f (%.f)',MeanData(p),SDData(p));
end
...
if Ntrials == 2
    ...
    close all;
    [comparison,~,~,~] = multcompare(stats,'ctype','bonferroni');                %   Column 6 is the p-value for each individual comparison.
    close all
